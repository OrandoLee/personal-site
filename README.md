...
- 回复 Orask：在独立工作台中筛选待回复留言、撰写邮件、发送给访客并查看回复历史。
...
这个页面与内容后台共用管理员登录状态。未登录时会先跳转到 `/login`，登录成功后自动返回回复工作台。

首次进入时需要设置发件邮箱。系统只把发件邮箱地址保存到 `SiteSetting`，SMTP 用户名和授权码仍由服务器环境变量管理，不会写入数据库。为避免部分邮箱服务商拒绝代发，建议发件邮箱与 `SMTP_USER` 保持一致。

回复流程：

- 左侧按“待回复 / 全部 / 已回复”筛选 Orask 留言，并支持搜索与刷新。
- 右侧展示访客原始留言、收件邮箱、邮件主题和正文编辑区。
- 每次发送先创建待发送记录；发送成功后更新为 `sent`，失败则保留为 `failed`，方便回看。
- 成功发送后，留言会自动标记已读并记录最后回复时间。
- 发件邮箱可在 `/dashboard/settings` 的“Orask 回复邮箱”中修改。

相关数据模型：

- `OraskMessage.repliedAt`：最后一次成功回复时间。
- `OraskReply`：保存收发邮箱、主题、正文、发送状态、失败原因和发送时间。
...
`SMTP_USER` 和 `SMTP_PASS` 同时用于 Orask 新留言通知与后台回复。`/login/ans` 首次设置的邮箱是邮件展示的发件地址，建议与 `SMTP_USER` 一致。
...
部署新版本前需要执行 `npm run db:deploy`，将 `OraskReply` 等 Prisma 迁移应用到生产数据库。
